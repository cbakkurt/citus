 ...
 */
void
PostprocessAlterExtensionCitusUpdateStmt(Node *node, bool
										 citusCanBeUpdatedToAvailableVersion)
{
	...
	/*
	 * If Citus version was already same with available version before standard
	 * process utility runs ALTER EXTENSION command, then Citus is not updated
	 * actually, no need to post process
	 */
...
MarkExistingObjectDependenciesDistributedIfSupported()
{
	...
	/* reorder object addresses list according to dependency order */
	List *sortedObjectAddresses = OrderObjectAddressListInDependencyOrder(
		resultingObjectAddresses);

	/* subtract the ones that are already in pg_dist_object */
	sortedObjectAddresses = list_difference(sortedObjectAddresses,
											distributedObjectAddressList);

	/*
	 * Make sure that no new nodes are added after this point until the end of the
	 * transaction by taking a RowShareLock on pg_dist_node, which conflicts with the
	 * ExclusiveLock taken by master_add_node.
	 * This guarantees that newly added nodes will have the object, because they will
	 * get it in master_add_node after this transaction finishes and the pg_dist_object
	 * record becomes visible.
	 */
	LockRelationOid(DistNodeRelationId(), RowShareLock);

	/* finally, mark the rest of the objects as distributed */
	foreach(listCell, sortedObjectAddresses)
	{
